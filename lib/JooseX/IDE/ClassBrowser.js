    ...
    Class("ClassBrowser", {
        ...
        methods: {
            ...
            selectedModule: function (select) {
                ...
                this.setTitle(module.meta.name);
            },
            ...
            selectedClass: function (select) {
                ...
                this.setTitle(c.meta.localName);
                this.setBody(html);
            },
            ...
            saveBody: function() {
                var namespace = this.selectedValueObject(this.moduleSelect());
                ...
                if (cat) {
                    if(cat.name == "Instance Methods") {
                        ...
                        clazz.meta.stem.properties.methods.openWithoutDeCompose();
                        clazz.meta.addMethod(this.getTitle(), bodyFunction);
                        clazz.meta.stem.properties.methods.close();
                        clazz.meta.getMethod(this.getTitle()).apply(clazz);
                    }
                    if(cat.name == "Attributes") {
                        var props;
                        eval("props = " + this.getBody());
                        clazz.meta.stem.properties.attributes.openWithoutDeCompose();
                        var init = props.init || null;
                        clazz.meta.addAttribute(this.getTitle(), init, props);
                        clazz.meta.stem.properties.attributes.close();
                        clazz.meta.getAttribute(this.getTitle()).apply(clazz);
                    }
                } else if (clazz) {
                    var fullname = namespace.meta.name + '.' + this.getTitle();
                    var props;
                    eval("props = " + this.getBody());
                    Class(fullname, props);
                } else if (namespace) {
                    Module(this.getTitle(), function(){});
                }
            },
            
            discardBody: function() {
                this.selectedElement(this.elementsSelect());
            },
            ...
            deleteBody: function() {
                ...
                if (cat) {
                    if(cat.name == "Instance Methods") {
                        clazz.meta.getMethod(this.getTitle()).unapply(clazz);
                        clazz.meta.stem.properties.methods.openWithoutDeCompose();
                        clazz.meta.removeMethod(this.getTitle());
                        clazz.meta.stem.properties.methods.close();
                    }
                    if(cat.name == "Attributes") {
                        var props;
                        eval("props = " + this.getBody());
                        clazz.meta.stem.properties.attributes.openWithoutDeCompose();
                        clazz.meta.getAttribute(this.getTitle()).unapply(clazz);
                        clazz.meta.removeAttribute(this.getTitle());
                        clazz.meta.stem.properties.attributes.close();
                    }
                } else if (clazz){
                    clazz.meta.parent.removeProperty(clazz.meta.localName);
                } else if (namespace) {
                    namespace.meta.parent.removeProperty(namespace.meta.localName);
                }
            },
