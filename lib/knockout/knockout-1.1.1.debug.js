...
(function () {
    ...
    ko.applyBindingsToNode = function (node, bindings, viewModel) {
        ...
        new ko.dependentObservable(
            ...
            null,
            { 'disposeWhen' : function () { return !ko.utils.domNodeIsContainedBy(node, node.ownerDocument ? node.ownerDocument : document); } }
        );
    ...
    'update': function (element, valueAccessor, allBindingsAccessor) {
        ...
        if (value) {
            ...
            for (var i = 0, j = value.length; i < j; i++) {
                var option = element.ownerDocument.createElement("OPTION");
                //
                //Hacked by Jan vl to allow functions to extract
                //optionsValue and optionsText
                //
                var optionValue, optionText;
                if (typeof allBindings['optionsValue'] == "string") {
                    optionValue = value[i][allBindings['optionsValue']];
                } else if (typeof allBindings['optionsValue'] == "function") {
                    optionValue = allBindings['optionsValue'](value[i]);
                } else {
                    optionValue = value[i];
                }
                
                //hacked by janvl to allow functions as option values 
                ...
                //hacked by Jan vl, see above
                if (typeof allBindings['optionsText'] == "string") {
                    optionText = value[i][allBindings['optionsText']];
                } else if (typeof allBindings['optionsText'] == "function") {
                    optionText = allBindings['optionsText'](value[i]);
                } else {
                    optionText = value[i];
                }
